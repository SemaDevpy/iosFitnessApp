//
//  StatsSquatsViewController.swift
//  FitnessApp
//
...
class StatsSquatsViewController: UIViewController {
    ...
    override func viewDidLoad() {
     ...
     func loadData(){
            ...
            for i in 0..<progressions.count{
                ...
                let dataEntry =  ChartDataEntry(x: Double(timeIntervalForDate), y: Double(progressions[i].totalSquats))
                dataEntries.append(dataEntry)
            }
        let chartDataSet = LineChartDataSet(entries: dataEntries, label: "Total Push Ups: \(progressions.last?.totalSquats ?? 1000)")
        ...
        func getProgressFromDataBase() ->  Results<Squats>{
            return realm.objects(Squats.self)
        }
    ...
    extension StatsSquatsViewController : IAxisValueFormatter{
